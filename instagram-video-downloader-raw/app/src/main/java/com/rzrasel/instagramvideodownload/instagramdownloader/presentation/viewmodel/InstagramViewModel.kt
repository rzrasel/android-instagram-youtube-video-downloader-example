            ...
            try {
                _uiState.value = when (val result = downloadUseCase(url)) {
                    ...
                    is DownloadState.Error -> {
                        val errorType = when {
                            result.message.contains("Invalid Instagram URL") -> InstagramUiState.ErrorType.InvalidUrl
                            result.message.contains("private") -> InstagramUiState.ErrorType.PrivateAccount
                            result.message.contains("blob URL") -> InstagramUiState.ErrorType.BlobUrl
                            result.message.contains("network") -> InstagramUiState.ErrorType.NetworkError
                            result.message.contains("not contain a video") -> InstagramUiState.ErrorType.NoVideoContent
                            else -> InstagramUiState.ErrorType.Unknown
                        }
                        InstagramUiState.DownloadError(result.message, errorType)
                    }
                ...
                _uiState.value = InstagramUiState.DownloadError(
                    ...
                    InstagramUiState.ErrorType.Unknown
                )
            }
        }
    }
